...
impl GBLinear {
    ...
    fn pred_many(&self, feats: ArrayView2<f32>, row_length: usize, gid: usize) -> Vec<f32> {
        let bias = self.bias(gid) as f32;
        let mut result = vec![];
        for feat in 0..row_length {
            let mut psum = bias;
            for fid in 0..self.mparam.num_feature {
                match feats.get((feat, fid)) {
                    None => {}
                    Some(fvalue) => {
...
impl GradBooster for GBLinear {
    ...
    fn predict_many(&self, feats: ArrayView2<f32>, row_length: usize, ntree_limit: usize) -> Vec<Vec<f32>> {
        (0..self.mparam.num_output_group)
